 ...
 * {@link #reorder(int...)}. Note that putAt() and {@link #removeAt(int)} do not run in constant
...
public class IndexedMap<K, V> implements SortedMap<K, V>, java.io.Serializable, Cloneable {
    ...
    private void tryCapacity(final long capacity) {
        final int needed = (int) Math.min(
                1 << 30,
                Math.max(2, CrossHash.nextPowerOfTwo((long) Math.ceil(capacity
                        / f))));
        if (needed > n)
            rehash(needed);
    }
    ...
    public boolean trim(final int n) {
        final int l = CrossHash.nextPowerOfTwo((int) Math.ceil(n / f));
        if (l >= n || size > maxFill(l, f)) return true;
        try {
            rehash(l);
        } catch (Exception cantDoIt) {
    ...
    */
    ...
    public static int arraySize(final int expected, final float f) {
        final long s = Math.max(2, CrossHash.nextPowerOfTwo((long) Math.ceil(expected / f)));
        if (s > (1 << 30))
