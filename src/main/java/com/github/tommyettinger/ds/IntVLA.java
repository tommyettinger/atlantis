...
public class IntVLA implements Serializable, Cloneable {
     ...
     */
    public IntVLA shuffle(Random random)
    {
        int n = size;
        for (int i = 0; i < n; i++)
        {
    ...
    @Override
    public int hashCode () {
        return CrossHash.hash(items, size);
    }
    
    /**
     * Hashes this IntVLA using {@link CrossHash}, getting a 64-bit result. The same algorithm is used by
     * {@link #hashCode()}, just with different constants and a different final step here.
     * @return a 64-bit hash code
     */
    public long hash64 () {
        return CrossHash.hash64(items, size);
    }
