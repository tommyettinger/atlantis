...
public class TimSort<T> {
	...
	/** The ordering being modified. */
	private IntVLA indices;
	...
	private int[] tmp; // Stores indices, like order
	...
	private TimSort(T[] a, IntVLA order, Comparator<? super T> c) {
		...
		int len = a.length;
		tmp = new int[len < 2 * INITIAL_TMP_STORAGE_LENGTH ? len >>> 1 : INITIAL_TMP_STORAGE_LENGTH];
	...
	public static <T> void sort (T[] a, IntVLA order, int lo, int hi, Comparator<? super T> c) {
		...
		do {
			// Identify next run
			int runLen = countRunAndMakeAscending(a, order, lo, hi, c);
	...
	private static <T> void binarySort (T[] a, IntVLA order, int lo, int hi, int start, Comparator<? super T> c) {
		...
		for (; start < hi; start++) {
			...
			while (left < right) {
				...
				if (c.compare(a[pivot], a[items[mid]]) < 0)
					right = mid;
				else
					left = mid + 1;
			}
			...
			switch (n) {
			case 2:
				items[left + 2] = items[left + 1];
			case 1:
				items[left + 1] = items[left];
				break;
			default:
				System.arraycopy(items, left, items, left + 1, n);
			}
			items[left] = pivot;
		}
	}
	...
	private static <T> int countRunAndMakeAscending (T[] a, IntVLA order, int lo, int hi, Comparator<? super T> c) {
		if (DEBUG) assert lo < hi;
		int runHi = lo + 1;
		if (runHi == hi) return 1;
		final int[] items = order.items;
		// Find end of run, and reverse range if descending
		if (c.compare(a[items[runHi++]], a[items[lo]]) < 0) { // Descending
			while (runHi < hi && c.compare(a[items[runHi]], a[items[runHi - 1]]) < 0)
				runHi++;
			reverseRange(items, lo, runHi);
		} else { // Ascending
			while (runHi < hi && c.compare(a[items[runHi]], a[items[runHi - 1]]) >= 0)
				runHi++;
		}
	...
	private static <T> int gallopLeft (T key, T[] a, int[] items, int base, int len, int hint, Comparator<? super T> c) {
		...
		if (c.compare(key, a[items[base + hint]]) > 0) {
			...
			int maxOfs = len - hint;
			while (ofs < maxOfs && c.compare(key, a[items[base + hint + ofs]]) > 0) {
				lastOfs = ofs;
				ofs = (ofs << 1) + 1;
		...
		while (lastOfs < ofs) {
			...
			if (c.compare(key, a[items[base + m]]) > 0)
	 ...
	 * @param key the key whose insertion point to search for
	 * @param a the array in which to search
	 * @param items the int array from inside the IntVLA to change the order in
	 * @param base the index of the first element in the range
	 * @param len the length of the range; must be > 0
	 * @param hint the index at which to begin the search, 0 <= hint < n. The closer hint is to the result, the faster this method
	 *           will run.
	...
	private static <T> int gallopRight (T key, T[] a, int[] items, int base, int len, int hint, Comparator<? super T> c) {
		if (DEBUG) assert hint >= 0 && hint < len;
		int ofs = 1;
		int lastOfs = 0;
		if (c.compare(key, a[items[base + hint]]) < 0) {
	...
	private void mergeLo (int base1, int len1, int base2, int len2) {
		...
		System.arraycopy(items, base1, tmp, 0, len1);
		...
		while (true) {
			...
			do {
				...
				count2 = gallopLeft(a[tmp[cursor1]], a, items, cursor2, len2, 0, c);
				if (count2 != 0) {
					System.arraycopy(items, cursor2, items, dest, count2);
					dest += count2;
					cursor2 += count2;
					len2 -= count2;
					if (len2 == 0) break outer;
	...
	private void mergeHi (int base1, int len1, int base2, int len2) {
		...
		System.arraycopy(items, base2, tmp, 0, len2);
		...
		while (true) {
			...
			do {
				...
				if (count1 != 0) {
					...
					cursor1 -= count1;
					len1 -= count1;
					System.arraycopy(items, cursor1 + 1, items, dest + 1, count1);
					if (len1 == 0) break outer;
				}
				items[dest--] = tmp[cursor2--];
				if (--len2 == 1) break outer;

				count2 = len2 - gallopLeft(a[items[cursor1]], a, tmp, 0, len2, len2 - 1, c);
				if (count2 != 0) {
					dest -= count2;
					cursor2 -= count2;
					len2 -= count2;
					System.arraycopy(tmp, cursor2 + 1, items, dest + 1, count2);
		...
		if (len2 == 1) {
			...
			cursor1 -= len1;
			System.arraycopy(items, cursor1 + 1, items, dest + 1, len1);
			items[dest] = tmp[cursor2]; // Move first elt of run2 to front of merge
		} else if (len2 == 0) {
			throw new IllegalArgumentException("Comparison method violates its general contract!");
		} else {
			if (DEBUG) assert len1 == 0;
